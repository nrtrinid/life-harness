...
**Status (mid-v2):** Living spec, typed handoffs, proof normalization, spec updates, next-slice adoption, headless `currentSlice` + phase machine, and the **first provider-runner next-job bridge** are landed. Next: risk-tier routing and UI consolidation around **Next Handoff**.
...
Feature Sprint generates markdown packets and accepts fenced JSON imports. It does **not** bind the app to a specific LLM provider. **Codex is not required** — Cursor, ChatGPT, Codex, local runner, manual paste, and optional DeepSeek automated review/prompt-audit are interchangeable worker/evaluator lanes.
...
Local runner bridges (Cursor CLI, optional Codex CLI) are **optional localhost helpers** — see [`feature-sprint-local-runner-v0.1.md`](feature-sprint-local-runner-v0.1.md). **DeepSeek** is a separate optional read-only evaluator lane (mock or server-side live) that stages **prompt critiques before implementation** and **review verdicts after implementation** — without repo mutation — see [`feature-sprint-deepseek-reviewer-v0.1.md`](feature-sprint-deepseek-reviewer-v0.1.md). Prompt audit uses **Pro by default**; Flash is deferred. Local models may **later** assist cheap structured tasks without replacing frontier scoping or auto-advance gates — see [`plans/feature-sprint-roadmap-v0.1.md`](plans/feature-sprint-roadmap-v0.1.md).
